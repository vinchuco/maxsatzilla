...
Main.C -- (C) Niklas Een, Niklas S�ensson, 2004
...
int 	 opt_lc=1;
int 	 opt_heur=3; // 0 NONE / 1 JEROSLOW / 2 VSIDS / 3 AUTO
int 	 opt_trans=2;
int      opt_sat=0;
int      opt_size=3;
int 	 opt_ls=1;
...
cchar* doc =
    "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n"
    "MiniMaxSAT 1.0, based on MiniSAT+ 1.0 -- (C) Federico Heras, 2006 ~~~~~~~~~~~~~\n"
    "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n"
    ...
    "  -F=0,-F=1,-F=2   Read a problem in Pseudo Boolean (0), CNF (1) or WCNF (2) format\n"
    "\n"
    ...
    //"  -L=0,-L=1     Apply local consistency (0-NONE / 1-UP) \n"
    //"  -H=0,1,2,3    Branching Heuristic (0- NONE / 1-JEROSLOW / 2-VSIDS / 3-AUTO) \n"
    //"  -t=0,-t=1     Transform or not the problem (0-NONE / 1-TRANSFORM) \n"
    //"  -l=0,-l=1     Apply Sthocastic Local Search (0-NONE / 1-SLS) \n"
    //"  -s=0,-s=1     0 = Optimization / 1 = Satisfiability \n"
    //"  -si=X         X specifies the maximum size of transformations (default 3)\n"
    //"  -S=0,-S=1     0 = None / 1 = Learning and Backjumping \n"
    //"  -P=0,1,2      0 = None / 1 = Preprocessing Probing / 2 = Probing \n"
    //"\n"
    //"Output options:\n"
    ...
    //"  -wcnf=<file>  Write Max-SAT problem to a file. Trivial UNSAT => no file written.\n"
    //"  -wcnf2=<file> Write Max-SAT problem to a file. Trivial UNSAT => no file written.\n"
    //"\n"
    "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n"
;
...
char* toString(Int num) { char buf[32]; sprintf(buf, "%d", num); return xstrdup(buf); }   // Caller must free string.
int   toint   (Int num) { if (num > INT_MAX || num < INT_MIN) throw Exception_IntOverflow(xstrdup("toint")); return (int)num; }
...
void parseOptions(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; i++){
	    ...
	    //else if (strncmp(arg, "-wcnf2="      ,   7) == 0) opt_wcnf2        = arg + 7;
	    else if (strncmp(arg, "-wcnf="      ,   6) == 0) opt_wcnf        = arg + 6;
	    ...
	    else if (strncmp(arg, "-s="      ,   4) == 0)
	    {
	       opt_sat=atoi(arg+ 3);
	    }
	    else if (strncmp(arg, "-si="      ,   4) == 0)
	    {
	       opt_size=atoi(arg+ 4);
	    }
	    else if (strncmp(arg, "-up="     ,   4) == 0) 
	    {
	    	opt_upper = atoi(arg+ 4);
	    }
            else
...
int main(int argc, char** argv)
{
	...
	if (opt_command == cmd_Minimize)
     ...
     {
     	// CNF or WCNF file
	if (opt_verbosity >= 1) reportf("Parsing CNF / WCNF file...\n");
	if(opt_file_type==ft_Cnf) parse_WCNF_file(opt_input, *pb_solver,true);
	else parse_WCNF_file(opt_input, *pb_solver,true);
	pb_solver->solveCnf(convert(opt_command));
