...
Global.h -- (C) Niklas Een, Niklas S�ensson, 2004
...
public:
   ...
   ~vec(void)                                                      { clear(true); }
    ...
    void     push_ (void)	       { if (sz == cap) {growAllTo(sz+1);} sz++;}
...
template<class T>    
void vec<T>::growAllTo(int size) {
    if (sz >= size) return;
    grow(size);
    for (int i = sz; i < cap; i++) { new (&data[i]) T();}
    }
