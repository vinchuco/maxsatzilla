...
sub run_sat_solver() {
	...
	`csh -f -c "limit cputime $time_limit; $csh_cmd" >& $tmpfile`;
    } else { 
	`csh -f -c "$csh_cmd" >& $tmpfile`;
    }
    # 3. Parse output file and get outcome
    my ($outcome, $assign) = &parse_sat_solver_output($outfile, $tmpfile);
...
sub compute_unsat_core() {
    ...
    my $DBGF = $ds->dbghandle;
    #
    my $coreref = &load_unsat_core($corefile);
    if (${$opts}{d}) {
	my $csize = $#{$coreref}+1;
